...
void _stdcall planparam_c(INTEGER *EersteDag, INTEGER *Hoeveel, INTEGER *PerWat, STRING *PerWatStr, INTEGER *PerDagen,
              INTEGER *MinSeparatie, INTEGER *MaxSeparatie, INTEGER *Duur, INTEGER *Enkel, INTEGER *DagMax, INTEGER *DvoorE);
...
{   /* NB er wordt nog maar ??????n oplossing verlangd! Redesign ??! */
TIJD     Plan;
JANEE    *Gewaarsch_1;
JANEE    *Gewaarsch_2;
CHAR     Fixed;
...
SEXLIST  *Sexlist;
WEDSCAT  VerliezersR;
INTEGER  EersteDag, DagMaxO, PerWat, PerDagen, MinSeparatie, MaxSeparatie, Duur, Enkel, DagMax, DvoorE;
STRING   PerWatStr;

  planparam_c(&EersteDag, &DagMaxO, &PerWat, &PerWatStr, &PerDagen, &MinSeparatie, &MaxSeparatie, &Duur, &Enkel, &DagMax, &DvoorE);
  ...
  for(TijdCurr = TijdArray; TijdCurr != TijdEnd ; TijdCurr++) { /* ga alle tijdstippen af */
    ...
    if (DagTypeC == wrk_w && (TijdCurr->Tijdstip & 0x7F) < UitKantMax)
      TijdCurr->BanenOver = 0;			/* schrap i.g.v. werkdag en v??????r uitkant */
  }
/*_______________________________________ alle planning _____________________________________*/
  if (!RUN_StackBTrack()) {             
	...
	if (TPers->SpNum == Dlnmr->SpNum) {
	  ...
	  Vlag++;       /* ??????n van de spelers speelt al elders */
	  if (ISpeler > 500) RUN_Exit(212); /* te veel ... */
	};
      };
    };
    if (Vlag) {		/* zet tijdstippen eromheen op verhinderd*/
      TijdCurr = binsearch(Plan - MinSeparatie + 1, TijdArray, TijdEnd);   /* de MinSeparatie */
      for(; (TijdCurr->Tijdstip < (Plan + MinSeparatie)) &&		/* de MinSeparatie */
						(TijdCurr != TijdEnd);
					TijdCurr++) {
	  TijdCurr->BanenOver = 0;
      };
...
#define reden_dubbel_voor_enkel   0x2000
#define reden_max_separatie       0x4000

void _stdcall check_forceer_c(INTEGER Mode, TIJDL *Tijden,
		  INTEGER WdNo, WEDSCAT Cat,  /* de wedstr zelf */
		/*  INTEGER MinSeparatie, INTEGER Duur, INTEGER Singles, INTEGER DagMax, INTEGER DagMaxO,*/
...
STRING   PerWatStr;
 ...
 /*_____________________________________________________________________*/
  ...
  for(PlanInd = 0; PlanInd < PlanAant ; PlanInd++) {
    ...
    if (  (PlanP+PlanInd)->Plan_Tijd < VorigPlan1 + MinSeparatie		/* MinSeparatie */
       || (PlanP+PlanInd)->Plan_Tijd < VorigPlan2 + MinSeparatie) {	/* MinSeparatie */
  ...
  /*_____________________________________________________________________*/
    DagNoC1 = DagNo - (DagNo - EersteDag) % PerDagen;
    DagNoC2 = DagNoC1 + PerDagen;
  if (!RUN_StackBTrack()) {             /* loop dooralle planning van de betreffende dag...    */
    w2_for_c_1(&WedsNo,&Wedscat,&Plan,&Gewaarsch_1,&Gewaarsch_2,&Fixed,&Vrij);
    if (PerWat == 0)
    ...
    Lower = Plan - MinSeparatie;	/* MinSeparatie */
    if (!Baan_Count) Lower = Plan - 1;     /* smerig maar bevalt het best .. -1 op 30.8.2007 voor dubbele boeking */
    Upper = Plan + MinSeparatie;	/* MinSeparatie */
    Top = GETSP;
    if (!RUN_StackBTrack()) {
      wd_for_c_3(WedsNo,Wedscat,&Tgnst1,&Tgnst2,&Uitslag,&Tijd1); /* wie doen er aan mee? */
      de_spelers(WedsNo,Wedscat,Tgnst1,Tgnst2,&Deelnemers);
      for (TPers = Spelers; TPers->fno==LISTFNO; TPers=TPers->next) {
      	for (Dlnmr = Deelnemers; Dlnmr->fno==LISTFNO; Dlnmr=Dlnmr->next) {
          if (TPers->SpNum == Dlnmr->SpNum) {
            ...
            (SpPlan + ISpeler)->Cat    = Wedscat;
            Top1 = GETSP;
            wc_for_c_1(Wedscat,&Wedsnaam,&Wedstyp,&Naamkort,&Sexlist,&Geld,&Verl); /* haal het wedstyp op */
            RUN_Cut(Top1);
            (SpPlan + ISpeler)->EDG = Wedstyp->fno;
            ISpeler++;
            for(PlanInd = 0 ; PlanInd < PlanAant ; PlanInd++) {
              ...
              if (MaxSeparatie > 0 && ((PlanP+PlanInd)->Plan_Tijd >> 7 == Plan >> 7)) {
                if ((PlanP+PlanInd)->Plan_Tijd - Plan > MaxSeparatie || Plan - (PlanP+PlanInd)->Plan_Tijd > MaxSeparatie) {
                  if (Write_Mode) {
                    sp_for_c_tel(TPers->SpNum,&Sex,&Naam,&Telefoon,&Telefoon2,&Telefoon3,&Verhinderd,&Offset,&Betaald,&UitKant,&Leeftijd);
                    Area += IO_SPrintf(Area,"\n%-20.20s => te veel tijd tussen wedstr. (%d)", Naam, MaxSeparatie);
                  }
                  (PlanP+PlanInd)->Plan_Bezwaren |= reden_max_separatie;
                } 
              }
              if (DvoorE > 0) {
                /*
                Area += IO_SPrintf(Area, "\n%d = Upper  %d = Lower %d = EDG_eigen , %d = Wedstyp->fno, %d = wedstyp_e", Upper, Lower, EDG_eigen, Wedstyp->fno, wedstyp_e);
                */
                if ((PlanP+PlanInd)->Plan_Tijd > Upper && Wedstyp->fno != wedstyp_e && EDG_eigen == wedstyp_e ||
                    (PlanP+PlanInd)->Plan_Tijd < Lower && Wedstyp->fno == wedstyp_e && EDG_eigen != wedstyp_e) {
                  (PlanP+PlanInd)->Plan_Bezwaren |= reden_dubbel_voor_enkel;

                  if (Write_Mode) {
                    sp_for_c_tel(TPers->SpNum,&Sex,&Naam,&Telefoon,&Telefoon2,&Telefoon3,&Verhinderd,&Offset,&Betaald,&UitKant,&Leeftijd);
                    Area += IO_SPrintf(Area,"\n%-20.20s => dubbel voor enkel.", Naam);
                  }
                }
              } /* DvoorE melden */
            }
  ...
  *TijdenStr = MEM_AllocGStack(100);
  for(PlanInd = 0 ; PlanInd < PlanAant ; PlanInd++) {
    ...
    if ((PlanP+PlanInd)->Plan_Bezwaren & reden_singles)
					(*TijdenStr)[DagNo++] = 's';
    if ((PlanP+PlanInd)->Plan_Bezwaren & reden_max_separatie)
					(*TijdenStr)[DagNo++] = 's';
...
void splitsc(int Ndelen, rlist *inlist, unsigned int OptVerh, unsigned int OptClub, unsigned int OptPlts, 
             unsigned int NotLoot34, unsigned int OptRating)
{
...
#ifdef mydebug2
  IO_Writef("\nGesorteerd:");
  for (ii = 0; ii <aantalGeplaatsten;ii++) {
    IO_Writef("\n Plaatsing: %d CatDelta: %d", opgaven[ii]->Plaatsing, opgaven[ii]->CatDelta);
    }
#endif
